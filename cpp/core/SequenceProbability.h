...
namespace sequence_probability {
    ...
    std::shared_ptr<Mat<typename model_t::value_t>> sequence_probabilities(
        ...
        log_prob->w.col(0) = G.rows_cols_pluck(out_state_and_prob.second, data.col(1), data_pluck)->w.row(0).transpose().array().log();
        ...
        for (int i = 1; i < n-1; i++) {
            ...
            if (i == 1) {
                log_prob->w.col(i).array() += plucked_activations->w.row(0).transpose().array().log();
            } else {
                log_prob->w.col(i).array() += plucked_activations->w.row(0).transpose().array().log() + log_prob->w.col(i).array();
            }
        }
