...
template<typename R>
vector<Mat<R>> StackedGatedModel<R>::parameters() const {
...
std::tuple<R, R> StackedGatedModel<R>::masked_predict_cost(
    ...
    for (uint i = 0; i < n-1; ++i) {
        ...
        logprobs      = this->decoder->activate(input_vector, std::get<1>(initial_state));
...
typename StackedGatedModel<R>::activation_t StackedGatedModel<R>::activate(
    ...
    auto memory       = gate.activate(input_vector, std::get<1>(previous_state)[this->stack_size-1]);
    input_vector      = input_vector.eltmul_broadcast_rowwise(memory);

    std::get<0>(out) = this->stacked_lstm->activate(previous_state, input_vector);
    std::get<1>(out) = softmax(this->decoder->activate(input_vector, std::get<1>(std::get<0>(out))));
