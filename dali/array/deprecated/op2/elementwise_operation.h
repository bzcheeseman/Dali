...
namespace op {
    ...
    expression::ExpressionGraph binary_kernel_function(
        const expression::ExpressionGraph& a,
        const expression::ExpressionGraph& b,
    ...
    expression::ExpressionGraph astype(const expression::ExpressionGraph& x, DType dtype);
    // static_cast one type to another. This can cause unpredictable
    // behavior on floats->integers based on underlying
    // hardware/implementation
    expression::ExpressionGraph unsafe_cast(const expression::ExpressionGraph& x, DType dtype);
    // Perform rounding on a value to nearest integer.
    // Note: equivalent to floor(x + 0.5).
    expression::ExpressionGraph round(const expression::ExpressionGraph& x);
