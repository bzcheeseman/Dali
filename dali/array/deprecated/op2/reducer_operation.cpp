...
namespace rtc {
struct ReducerExpressionNode : public RtcExpression {
    ...
    // DO NOT REIMPLEMENT
    ReducerExpressionNode(const std::string& functor_name,
...
};  // struct ReducerExpressionNode

struct AllReducerExpressionNode : public ReducerExpressionNode {
    ...
    AllReducerExpressionNode(const std::string& functor_name, std::shared_ptr<const RtcExpression> argument);
...
struct AxisReducerExpressionNode : public ReducerExpressionNode {
    static const hash_t optype_hash_cache_;
    ...
    AxisReducerExpressionNode(const std::string& functor_name, std::shared_ptr<const RtcExpression> argument);
...
struct ArgumentAllReducerExpressionNode : public AllReducerExpressionNode {
    static const hash_t optype_hash_cache_;

    using AllReducerExpressionNode::AllReducerExpressionNode;
...
struct ArgumentAxisReducerExpressionNode : public AxisReducerExpressionNode {
    static const hash_t optype_hash_cache_;

    using AxisReducerExpressionNode::AxisReducerExpressionNode;
...
const hash_t AxisReducerExpressionNode::optype_hash_cache_ = std::hash<std::string>()("AxisReducerExpressionNode");

AllReducerExpressionNode::AllReducerExpressionNode(
        const std::string& functor_name,
        std::shared_ptr<const RtcExpression> argument) :
    ReducerExpressionNode(functor_name, argument, 1) {
}

void AllReducerExpressionNode::compute_node_compilation_info(
        int desired_computation_rank,
        const std::vector<int>& desired_computation_shape,
        std::vector<const RtcArrayWrapper*>* arrays,
        std::vector<const ScalarWrapper*>* scalars,
...
std::string AllReducerExpressionNode::prefix_code(
        const node_to_info_t& node_to_info,
        memory::DeviceT device_type) const {
...
const hash_t ArgumentAllReducerExpressionNode::optype_hash_cache_ = std::hash<std::string>()("ArgumentAllReducerExpressionNode");

AxisReducerExpressionNode::AxisReducerExpressionNode(
        const std::string& functor_name,
        std::shared_ptr<const RtcExpression> argument) :
    ReducerExpressionNode(functor_name, argument, std::max(argument->min_computation_rank_ - 1, 1)) {
}

std::vector<int> AxisReducerExpressionNode::bshape() const {
...
void AxisReducerExpressionNode::compute_node_compilation_info(
        int desired_computation_rank,
        const std::vector<int>& desired_computation_shape,
        std::vector<const RtcArrayWrapper*>* arrays,
        std::vector<const ScalarWrapper*>* scalars,
...
bool AxisReducerExpressionNode::is_dim_collapsible_with_dim_minus_one(
        const int& dim) const {
    return argument_->is_dim_collapsible_with_dim_minus_one(dim - 1);;
}

std::shared_ptr<const RtcExpression> AxisReducerExpressionNode::collapse_dim_with_dim_minus_one(
        const int& dim) const {
    return std::make_shared<AxisReducerExpressionNode>(
        functor_name_,
        argument_->collapse_dim_with_dim_minus_one(dim - 1)
    );
}

std::shared_ptr<const RtcExpression> AxisReducerExpressionNode::transpose(
...
std::string AxisReducerExpressionNode::prefix_code(
        const node_to_info_t& node_to_info,
        memory::DeviceT device_type) const {
...
std::string ArgumentAllReducerExpressionNode::prefix_code(
        const node_to_info_t& node_to_info,
        memory::DeviceT device_type) const {
...
const hash_t ArgumentAxisReducerExpressionNode::optype_hash_cache_ = std::hash<std::string>()("ArgumentAxisReducerExpressionNode");

std::string ArgumentAxisReducerExpressionNode::prefix_code(
        const node_to_info_t& node_to_info,
        memory::DeviceT device_type) const {
    return create_argument_axis_reduce_kernel_caller(node_to_info.at(argument_.get()).computation_rank);
}

std::shared_ptr<const RtcExpression> ArgumentAxisReducerExpressionNode::collapse_dim_with_dim_minus_one(
        const int& dim) const {
    return std::make_shared<ArgumentAxisReducerExpressionNode>(
        functor_name_,
        argument_->collapse_dim_with_dim_minus_one(dim - 1)
    );
}

std::shared_ptr<const RtcExpression> ArgumentAxisReducerExpressionNode::transpose(
        const std::vector<int>& permutation) const {
    auto new_permutation = permutation;
    // add last dim of tensor with rank (permutation.size() + 1)
    new_permutation.emplace_back(permutation.size());
...
const hash_t AllReducerExpressionNode::optype_hash_cache_ =
        std::hash<std::string>()("AllReducerExpressionNode");
...
namespace op {
    expression::ExpressionGraph all_reduce(
            const expression::ExpressionGraph& a,
            const std::string& reducer_name) {
        return expression::ExpressionGraph(std::make_shared<expression::rtc::AllReducerExpressionNode>(
            reducer_name,
            a.state_->as_jit()
        ));
    }

    expression::ExpressionGraph axis_reduce(
            const expression::ExpressionGraph& a,
            const std::string& reducer_name,
        ...
        if (num_low_axes_to_reduce > 0) {
            ...
            for (int axes_used_up = 0; axes_used_up < num_low_axes_to_reduce; ++axes_used_up) {
                if (num_low_axes_to_reduce - axes_used_up == 1) {
                    res = std::make_shared<expression::rtc::AxisReducerExpressionNode>(
                        reducer_name,
                        res
                    );
                } else {
        ...
        return expression::ExpressionGraph(res);
    }

    expression::ExpressionGraph argument_all_reduce(
            const expression::ExpressionGraph& a,
            const std::string& reducer_name) {
        return expression::ExpressionGraph(std::make_shared<expression::rtc::ArgumentAllReducerExpressionNode>(
            reducer_name,
            a.state_->as_jit()
        ));
    }

    expression::ExpressionGraph argument_axis_reduce(
            const expression::ExpressionGraph& a,
            const std::string& reducer_name,
            const int& axis) {
        int ndim = a.ndim();
        if (ndim == 0) return expression::ExpressionGraph(0);
        ...
        return expression::ExpressionGraph(std::make_shared<expression::rtc::ArgumentAxisReducerExpressionNode>(
            reducer_name,
            res
        ));
    }
