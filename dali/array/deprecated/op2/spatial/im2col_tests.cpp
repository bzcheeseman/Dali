...
TEST(RTCTests, im2col_without_channels) {
    ...
    Array rtc_res_nhwc = op::im2col(image_nhwc, 3, 3, 1, 1, 0, 0, "NHWC");
    EXPECT_TRUE(Array::equals(rtc_res_nhwc, old_res_nhwc));

    Array old_res_nchw = old_op::im2col(
        image_nchw, 3, 3, 1, 1, "NCHW"
    );
    Array rtc_res_nchw = op::im2col(image_nchw, 3, 3, 1, 1, 0, 0, "NCHW");
    ...
    Array rtc_res_wnch = op::im2col(image_wnch, 3, 3, 1, 1, 0, 0, "WNCH");
