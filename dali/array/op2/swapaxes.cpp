...
namespace rtc {
struct SwapaxesExpressionState : public RtcExpression {
    ...
    SwapaxesExpressionState(std::shared_ptr<const RtcExpression> input,
                            int axis1,
                            int axis2) :
    ...
    std::shared_ptr<const RtcExpression> collapse_dim_with_dim_minus_one(const int& dim) const {
        return std::make_shared<SwapaxesExpressionState>(
...
const hash_t SwapaxesExpressionState::optype_hash = std::hash<std::string>()("SwapaxesExpressionState");
}  // namespace rtc
}  // namespace expression
...
namespace op {
    expression::Expression swapaxes(const expression::Expression& input, int axis1, int axis2) {
        ...
        if (auto input_as_array = input.state_->as_array()) {
            ...
            return expression::Expression(
                std::make_shared<expression::rtc::SwapaxesExpressionState>(
                    input_as_jit,
