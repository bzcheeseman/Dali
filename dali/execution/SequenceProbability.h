...
namespace sequence_probability {
    ...
    //     MAT(log_prob).col(0) = MAT(std::get<1>(out_state_and_prob)(
    //         data.col(1),
    //         data_pluck
    ...
    //             data_pluck
    //         );
    //         MAT(log_prob).col(i).array() += MAT(plucked_activations).row(0).transpose().array().log() + MAT(log_prob).col(i-1).array();
    //     }
