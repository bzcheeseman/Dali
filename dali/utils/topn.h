#ifndef DALI_UTILS_TOPN_H
#define DALI_UTILS_TOPN_H
#include <algorithm>
#include <functional>
#include <vector>
...
namespace utils {
    ...
    const T &TopN<T, Cmp>::peek_bottom() {
        ASSERT2(!empty(), "peek_bottom called on empty TopN.");
        if (state_ == UNORDERED) {
    ...
    void TopN<T, Cmp>::ExtractNondestructive(std::vector<T> *output) const {
        ASSERT2(output != NULL, "called ExtractNondestructive with NULL ptr.");
        *output = elements_;
        if (state_ != HEAP_SORTED) {
    ...
    void TopN<T, Cmp>::ExtractUnsortedNondestructive(std::vector<T> *output) const {
        ASSERT2(output != NULL, "called ExtractUnsortedNondestructive with NULL ptr.");
        *output = elements_;
        if (state_ == HEAP_SORTED) {
            // Remove the limit_+1'th element.
            output->pop_back();
